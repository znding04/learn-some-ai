...
### The Threat Landscape and Automation
...
Machine learning for cybersecurity generally falls into three broad categories.
...
### AI-Native Security and a Brief History
...
**Threat landscape**
The full range of cyber threats facing an organization, from commodity malware to targeted APTs. Understanding the threat landscape involves mapping attack surfaces, identifying likely adversaries, and continuously monitoring for emerging threats and vulnerabilities.

**MITRE ATT&CK framework**
A structured knowledge base mapping adversary tactics, techniques, and procedures (TTPs) across the attack lifecycle. It provides a common vocabulary for describing adversarial behavior and enables AI systems to classify and correlate attack behaviors systematically.

**Supervised ML for security**
Classification of known threat types (malware, phishing, spam) using labeled training data. Models learn to distinguish between benign and malicious samples by extracting features from executables, network traffic, or email content.

**Unsupervised ML for security**
Anomaly detection to surface unknown threats by learning normal behavior baselines. Unlike supervised approaches that require labeled data, unsupervised methods detect deviations from expected patterns, making them effective against novel attacks.

**Reinforcement learning for adaptive defense**
Agents that learn dynamic security policies through interaction with simulated environments. An RL agent can learn to reconfigure defenses, allocate resources, and respond to attacks by optimizing a reward function tied to security outcomes.

**AI-native security**
Designing security architectures with AI at the core, including continuous retraining and adversarial robustness. This approach integrates AI capabilities from the ground up rather than bolting ML models onto legacy systems after the fact.

**Concept drift**
The shift in data distributions over time that degrades model accuracy, a critical challenge in malware and intrusion detection. As attackers evolve their techniques, the statistical properties of attack data change, requiring models to be continuously retrained or adapted.

**Adversarial robustness**
Ensuring ML models resist evasion by adversaries who craft inputs to fool classifiers. Evaluated with tools like CleverHans, adversarial robustness is essential in security applications where attackers actively try to bypass detection systems.
