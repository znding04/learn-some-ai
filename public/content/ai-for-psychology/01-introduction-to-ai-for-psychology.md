...
- **ELIZA effect**: The tendency for humans to attribute understanding and empathy to computer programs that produce human-like responses, even when the program has no actual comprehension.
- **Computational psychiatry**: A subfield that uses mathematical and computational models to understand, predict, and treat psychiatric disorders, bridging neuroscience, psychology, and AI.
- **Digital phenotyping**: The moment-by-moment quantification of individual-level human behavior using data from smartphones and wearable devices, including GPS, accelerometer, typing patterns, and call/text logs.
- **Ecological momentary assessment (EMA)**: A method of collecting real-time self-report data from participants in their natural environments, often via smartphone prompts, reducing recall bias inherent in retrospective questionnaires.
- **Therapeutic alliance**: The collaborative relationship between therapist and client, considered a strong predictor of therapy outcome -- a key factor when evaluating whether AI systems can substitute for or complement human therapists.
- **Reinforcement learning (RL)**: A machine learning paradigm where an agent learns to make sequential decisions by maximizing cumulative reward, applied in psychology for adaptive treatment strategies.
